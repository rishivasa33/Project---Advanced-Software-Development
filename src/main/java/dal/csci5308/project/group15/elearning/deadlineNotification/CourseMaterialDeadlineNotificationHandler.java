    ...
    public List<CourseMaterialDeadlineNotification> getCourseMaterialDeadlineNotifications()
    {
        ...
        try
        {
            ...
            for (int row = 0; row < databaseOperations.getRowCount(resultSet); row++)
            {
                ...
                notification.setSubjectId(String.valueOf(databaseOperations.getValueAt(resultSet, "MATERIAL_ID", row)));
                notification.setSubjectId(String.valueOf(databaseOperations.getValueAt(resultSet, "DESCRIPTION_TEXT", row)));
                notification.setSubjectId(String.valueOf(databaseOperations.getValueAt(resultSet, "START_DATE", row)));
                notification.setSubjectId(String.valueOf(databaseOperations.getValueAt(resultSet, "END_DATE", row)));
                notification.setSubjectId(String.valueOf(databaseOperations.getValueAt(resultSet, "NUM_DAYS", row)));
